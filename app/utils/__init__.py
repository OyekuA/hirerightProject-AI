...
def parse_gemini_json(generated: str) -> Union[dict, list]:
    ...
    logger.debug("Parsing Gemini response", length=len(generated), hash=payload_hash)
    ...
    for match in matches:
        try:
            ...
            continue
    payload_hash = hashlib.sha256(generated.encode()).hexdigest()[:8]
    logger.error("Failed to extract JSON from Gemini response", length=len(generated), hash=payload_hash)
    raise json.JSONDecodeError("Could not parse JSON from Gemini response", "[REDACTED]", 0)